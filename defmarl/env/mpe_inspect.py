...
class MPEInspect(MultiAgentEnv, ABC):
    ...
    def __init__(
        ...
        area_size = MPEInspect.PARAMS["default_area_size"]
        max_step = 600  # 30 seconds
        dt = 0.05
        super(MPEInspect, self).__init__(num_agents, area_size, max_step, max_travel, dt, params)
        if self.params["n_obs"] != 2:
            self.params["n_obs"] = 2
            print("WARNING: n_obs is set to 2 for MPEInspect.")
    ...
    # def cost_components(self) -> Tuple[str, ...]:
    #     return "agent collisions", "target collisions", "obs collisions", "extreme collisions"
    ...
    # def non_rl_agent_actions(self,non_rl_agent_states: AgentState) -> Action:
    #     non_rl_agent_actions = jnp.zeros((self.non_rl_agents, self.action_dim), dtype=jnp.float32)
    #     non_rl_agent_actions = non_rl_agent_actions.at[:,0].set(-2/9 * (non_rl_agent_states[:, 0] - 2.0)) #-w^2 * x
    #     non_rl_agent_actions = non_rl_agent_actions.at[:,1].set(-8/9 * (non_rl_agent_states[:, 1] - 2.0)) #-4 * w^2 * y
    #     return non_rl_agent_actions
    ...
    def get_reward(self, graph: MPEEnvGraphsTuple, action: Action) -> Reward:
        ...
        # jax.debug.print("blocked: {}",blocked)
        # jax.debug.print("blocked.shape: {}",blocked.shape)

        # Mask visibility
        is_visible = jnp.logical_and(is_visible, jnp.logical_not(blocked))
        # jax.debug.print("is_visible_after: {}",is_visible)
        # jax.debug.print("is_visible_after.shape: {}",is_visible.shape)
        ################## visibility check ##################
    ...
    def get_cost(self, graph: MPEEnvGraphsTuple) -> Cost:
        ...
        non_rl_agent_pos = jnp.concatenate([
            non_rl_agent_pos,
            non_rl_agent_pos + non_rl_agent_states[:,2:]/jnp.linalg.norm(non_rl_agent_states[:,2:], axis=-1, keepdims=True) * self.params["obs_radius"]/3*5,  # in front
            non_rl_agent_pos - non_rl_agent_states[:,2:]/jnp.linalg.norm(non_rl_agent_states[:,2:], axis=-1, keepdims=True) * self.params["obs_radius"]/3*5  # behind
        ], axis=0)
        ...
        #agent should not be too close to the edge of the area
        # edges = jnp.array([[0., 0.], [0., self.area_size], [self.area_size, 0.], [self.area_size, self.area_size]])
        # dist = jnp.linalg.norm(jnp.expand_dims(agent_pos, 1) - jnp.expand_dims(edges, 0), axis=-1)
        # min_dist = jnp.min(dist, axis=1)
        # edge_cost: Array = self.params["car_radius"] - min_dist

        # cost = jnp.concatenate([agent_cost[:, None], target_cost[:, None], obs_cost[:, None], edge_cost[:, None]], axis=1)
    ...
    def render_video(
        ...
        # plot obstacles
        if self.params["n_obs"] > 0:
            obs_pos = graph0.type_states(type_idx=MPEInspect.OBS, n_type=self.params["n_obs"])[:, :2]  # [n_obs, 2]
        ...
        # plot agents
        n_pos = np.array(graph0.type_states(type_idx=MPEInspect.AGENT, n_type=self.num_agents)[:, :2]).astype(np.float32)
        agent_circs = [
        ...
        # plot goals
        goal_pos = np.array(graph0.type_states(type_idx=MPEInspect.GOAL, n_type=n_goal)[:, :2]).astype(np.float32)
        goal_circs = [
            plt.Circle((float(goal_pos[i, 0]), float(goal_pos[i, 1])), self.params["car_radius"],
                       color=goal_color, linewidth=0.0)
            for i in range(n_goal)
        ]
        goal_col = MutablePatchCollection(goal_circs, match_original=False, zorder= 6)
        ax.add_collection(goal_col)

        #plot goal-reach-avoid-region
        non_rl_agent_states = graph0.type_states(type_idx=MPEInspect.GOAL, n_type=n_goal * 3)
        non_rl_agent_pos = non_rl_agent_states[:, :2]
        ...
        visibility_text = ax.text(0.5, 0.99, "Visibility: []", va="top", ha="center", **text_font_opts)
        ...
        def update(kk: int) -> list[plt.Artist]:
            ...
            goal_pos_t = np.array(graph.type_states(type_idx=MPEInspect.GOAL, n_type=n_goal)[:, :2]).astype(np.float32)
            for ii in range(n_goal):
                goal_circs[ii].set_center(tuple(goal_pos_t[ii]))
            
            # update goal-reach-avoid-region
            non_rl_agent_states_t = np.array(graph.type_states(type_idx=MPEInspect.GOAL, n_type=n_goal))
            non_rl_agent_pos_t = non_rl_agent_states_t[:, :2]
            non_rl_agent_updates = np.concatenate([
                non_rl_agent_pos_t,
                non_rl_agent_pos_t + non_rl_agent_states_t[:,2:]/np.linalg.norm(non_rl_agent_states_t[:,2:], axis=-1, keepdims=True) * self.params["obs_radius"]/3*5,  # in front
                non_rl_agent_pos_t - non_rl_agent_states_t[:,2:]/np.linalg.norm(non_rl_agent_states_t[:,2:], axis=-1, keepdims=True) * self.params["obs_radius"]/3*5  # behind
            ], axis=0)
            ...
            # update visibility with the same logic as get_reward
            # RL and non-RL agent positions
            non_rl_agent_pos = non_rl_agent_pos_t  # [n_non_rl, 2]
            agent_pos = n_pos_t                # [n_rl, 2]
            obs_pos = np.array(graph.type_states(type_idx=MPEInspect.OBS, n_type=self.params["n_obs"]))[:, :2]
            pos_diff = non_rl_agent_pos[:, None, :] - agent_pos[None, :, :]  # [n_non_rl, n_rl, 2]
            # jax.debug.print("non_rl_agent_pos: {}, agent_pos: {}, pos_diff: {}",non_rl_agent_pos.shape,agent_pos.shape,pos_diff.shape)

            # Visibility by distance
            is_visible = jnp.linalg.norm(pos_diff, axis=-1) < 1.0  # [n_non_rl, n_rl]
            # jax.debug.print("is_visible: {}",is_visible.shape)
            ...
            blocked = jnp.any(obs_blocking, axis=0)  # [n_non_rl, n_rl]
            blocked = blocked.squeeze() 
            # jax.debug.print("blocked: {}",blocked.shape)
            is_visible = jnp.logical_and(is_visible, jnp.logical_not(blocked))
            # jax.debug.print("is_visible_after: {}",is_visible.shape)
            visibility_text.set_text(f"Visibility: {is_visible}")   
    ...
    def get_graph(self, env_state: MPEEnvState) -> MPEEnvGraphsTuple:
        ...
        node_type = node_type.at[:self.num_agents].set(MPEInspect.AGENT)
        node_type = node_type.at[self.num_agents: self.num_agents + self.non_rl_agents].set(MPEInspect.GOAL)
        if self.params["n_obs"] > 0:
            node_type = node_type.at[self.num_agents + self.non_rl_agents:].set(MPEInspect.OBS)
