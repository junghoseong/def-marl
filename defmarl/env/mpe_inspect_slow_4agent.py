...
class MPEInspect_Slow_4Agent(MultiAgentEnv, ABC):
    ...
    def __init__(
        ...
        area_size = MPEInspect_Slow_4Agent.PARAMS["default_area_size"]
        max_step = 1200  # 60 seconds
        dt = 0.05
        super(MPEInspect_Slow_4Agent, self).__init__(num_agents, area_size, max_step, max_travel, dt, params)
    ...
    def render_video(
        ...
        # plot obstacles
        if self.params["n_obs"] > 0:
            obs_pos = graph0.type_states(type_idx=MPEInspect_Slow_4Agent.OBS, n_type=self.params["n_obs"])[:, :2]  # [n_obs, 2]
        ...
        # plot agents
        n_pos = np.array(graph0.type_states(type_idx=MPEInspect_Slow_4Agent.AGENT, n_type=self.num_agents)[:, :2]).astype(np.float32)
        agent_circs = [
        ...
        # plot goals
        goal_pos = np.array(graph0.type_states(type_idx=MPEInspect_Slow_4Agent.GOAL, n_type=n_goal)[:, :2]).astype(np.float32)
        goal_circs = [
        ...
        rl_agent_states = graph0.type_states(type_idx=MPEInspect_Slow_4Agent.AGENT, n_type=self.num_agents)
        ...
        non_rl_agent_states = graph0.type_states(type_idx=MPEInspect_Slow_4Agent.GOAL, n_type=n_goal * 3)
        non_rl_agent_pos = non_rl_agent_states[:, :2]
        ...
        def update(kk: int) -> list[plt.Artist]:
            ...
            goal_pos_t = np.array(graph.type_states(type_idx=MPEInspect_Slow_4Agent.GOAL, n_type=n_goal)[:, :2]).astype(np.float32)
            ...
            rl_agent_states_t = np.array(graph.type_states(type_idx=MPEInspect_Slow_4Agent.AGENT, n_type=self.num_agents))
            ...
            non_rl_agent_states_t = np.array(graph.type_states(type_idx=MPEInspect_Slow_4Agent.GOAL, n_type=n_goal))
    ...
    def get_graph(self, env_state: MPEEnvState) -> MPEEnvGraphsTuple:
        ...
        node_type = node_type.at[:self.num_agents].set(MPEInspect_Slow_4Agent.AGENT)
        node_type = node_type.at[self.num_agents: self.num_agents + self.non_rl_agents].set(MPEInspect_Slow_4Agent.GOAL)
        if self.params["n_obs"] > 0:
            node_type = node_type.at[self.num_agents + self.non_rl_agents:].set(MPEInspect_Slow_4Agent.OBS)
