...
class MPEInspect_BiggerObstacle(MultiAgentEnv, ABC):
    ...
    def __init__(
        ...
        area_size = MPEInspect_BiggerObstacle.PARAMS["default_area_size"]
        max_step = 600  # 30 seconds
        dt = 0.05
        super(MPEInspect_BiggerObstacle, self).__init__(num_agents, area_size, max_step, max_travel, dt, params)
        if self.params["n_obs"] != 2:
            self.params["n_obs"] = 2
            print("WARNING: n_obs is set to 2 for MPEInspect_BiggerObstacle.")
    ...
    def get_reward(self, graph: MPEEnvGraphsTuple, action: Action) -> Reward:
        ...
        t = jnp.sum(oa * ba, axis=-1, keepdims=True) / (ba_norm_sq + 1e-8)  # [n_obs, n_rl, 1]
        obs_in_segment = jnp.logical_and(t >= 0.0, t <= 1.0)  # [n_obs, n_rl, 1]
        obs_in_segment = obs_in_segment.squeeze(-1)           # [n_obs, n_rl]
        ...
        cross = pos_diff_exp[..., 0] * obs_diff[..., 1] - pos_diff_exp[..., 1] * obs_diff[..., 0]
        ...
        obs_blocking = jnp.logical_and(obs_to_line, obs_in_segment[:, None, :])  # [n_obs, n_non_rl, n_rl]
        ...
        reward = jax.lax.cond(
            ...
            lambda _: jnp.array(-0.1 - 0.1 * (min_dist - agent_fov_range), dtype=jnp.float32), #########
            operand=None
        )
        # Action penalty
    ...
    def render_video(
        ...
        # plot obstacles
        if self.params["n_obs"] > 0:
            obs_pos = graph0.type_states(type_idx=MPEInspect_BiggerObstacle.OBS, n_type=self.params["n_obs"])[:, :2]  # [n_obs, 2]
        ...
        # plot agents
        n_pos = np.array(graph0.type_states(type_idx=MPEInspect_BiggerObstacle.AGENT, n_type=self.num_agents)[:, :2]).astype(np.float32)
        agent_circs = [
        ...
        # plot goals
        goal_pos = np.array(graph0.type_states(type_idx=MPEInspect_BiggerObstacle.GOAL, n_type=n_goal)[:, :2]).astype(np.float32)
        goal_circs = [
        ...
        non_rl_agent_states = graph0.type_states(type_idx=MPEInspect_BiggerObstacle.GOAL, n_type=n_goal * 3)
        non_rl_agent_pos = non_rl_agent_states[:, :2]
        ...
        def update(kk: int) -> list[plt.Artist]:
            ...
            goal_pos_t = np.array(graph.type_states(type_idx=MPEInspect_BiggerObstacle.GOAL, n_type=n_goal)[:, :2]).astype(np.float32)
            for ii in range(n_goal):
                goal_circs[ii].set_center(tuple(goal_pos_t[ii]))
            
            # update goal-reach-avoid-region
            non_rl_agent_states_t = np.array(graph.type_states(type_idx=MPEInspect_BiggerObstacle.GOAL, n_type=n_goal))
    ...
    def get_graph(self, env_state: MPEEnvState) -> MPEEnvGraphsTuple:
        ...
        node_type = node_type.at[:self.num_agents].set(MPEInspect_BiggerObstacle.AGENT)
        node_type = node_type.at[self.num_agents: self.num_agents + self.non_rl_agents].set(MPEInspect_BiggerObstacle.GOAL)
        if self.params["n_obs"] > 0:
            node_type = node_type.at[self.num_agents + self.non_rl_agents:].set(MPEInspect_BiggerObstacle.OBS)
